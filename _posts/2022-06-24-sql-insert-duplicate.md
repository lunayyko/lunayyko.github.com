...
replace into를 사용하고 싶었는데 pk를 intact하게 놔두기 위해서 duplicate key update 뒤에 중복시 업데이트할 행을 명시해주었다.
sql에서는 변수명으로 저렇게 아름답게 해줄 수 없어서 아래와같이 작성했다.  
머리가 나쁘면 몸이 고생한다고 하는데 노가다가 이렇게 적성에 잘 맞는지 개발을 하면서 깨닫고 있다.
