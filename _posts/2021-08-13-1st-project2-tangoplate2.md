...
프로젝트를 진행하다보니 뷰를 리소스 기준으로 나누는 것이 URI를 보면 이게 어떤 기능을 할 지 알 수 있는 API(restful한 API)를 작성하기 좋았다. 
예를 들어서 처음에는 리뷰 앱을 따로 만들었는데 사실  
리뷰 페이지는 식당 세부 정보 밑에 붙어있었으니, 리뷰를 식당 앱에 같이 넣었으면 좋았겠다고 생각했다.
아래의 구조가 최적화된 뷰의 구조일 것 같다. 
